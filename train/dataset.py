...
from data.filelock_rdt import FileLock_RDT
...
from data.directory_vla_dataset import DirectoryVLADataset as HDF5VLADataset
...
def reset_chunk_dirty_bits(chunk_dir, chunk_size):
    """
    Reset the dirty bit file for a specific chunk directory.
    """
    try:
        file_path = os.path.join(chunk_dir, "dirty_bit")
        lock = FileLock_RDT(file_path)
        lock.acquire_write_lock()
        try:
            # Create fresh dirty bit array (all clean)
        ...
        print(f"Error resetting dirty bits for {chunk_dir}: {e}")
        return False
...
class VLAConsumerDataset(Dataset):
    ...
    def __init__(
        ...
        #     self.empty_lang_embed = torch.load("data/pickup_apple.pt")
    ...
    def __len__(self) -> int:
        """
        Modified to return length based on available chunks rather than total possible chunks
        """

        if self.hdf5_dataset:
            return self.hdf5_dataset.get_totol_episode_lengths()
        # else:
        #     if not hasattr(self, '_available_chunks'):
        #         # This will trigger the scan for available chunks
        #         self._safe_load(0)
        #     return len(self._available_chunks) * self.chunk_size

    # def _safe_load(self, index):
    #     """
    #     Modified _safe_load method for VLAConsumerDataset with improved chunk handling.
    #     """
    #     if not hasattr(self, '_available_chunks'):
    #         # Scan for available chunks once at the beginning
    #         self._available_chunks = []
    #         for chunk_idx in range(self.num_chunks):
    #             chunk_dir = os.path.join(self.buffer_dir, f"chunk_{chunk_idx}")
    #             if os.path.exists(chunk_dir):
    #                 try:
    #                     if os.path.exists(os.path.join(chunk_dir, "dirty_bit")):
    #                         self._available_chunks.append(chunk_idx)
    #                 except:
    #                     continue
    #
    #         if not self._available_chunks:
    #             raise RuntimeError("No valid chunks found in buffer directory")
    #         print(f"Found {len(self._available_chunks)} valid chunks: {self._available_chunks}")
    #
    #     # Try to find a chunk with clean items
    #     chunks_tried = set()
    #     chunk_idx_position = index % len(self._available_chunks)
    #
    #     while len(chunks_tried) < len(self._available_chunks):
    #         read_chunk_idx = self._available_chunks[chunk_idx_position]
    #         read_chunk_dir = os.path.join(self.buffer_dir, f"chunk_{read_chunk_idx}")
    #         chunks_tried.add(read_chunk_idx)
    #
    #         try:
    #             read_chunk_item_indices = get_clean_item(read_chunk_dir)
    #             if read_chunk_item_indices:  # If we found clean items
    #                 break
    #         except Exception as e:
    #             print(f"Error reading chunk {read_chunk_idx}: {e}")
    #             read_chunk_item_indices = []
    #
    #         # Move to next chunk
    #         chunk_idx_position = (chunk_idx_position + 1) % len(self._available_chunks)
    #
    #     # If no clean items found in any chunk, reset all chunks
    #     if not read_chunk_item_indices:
    #         print("No clean items found. Resetting dirty bits for all chunks...")
    #         for chunk_idx in self._available_chunks:
    #             chunk_dir = os.path.join(self.buffer_dir, f"chunk_{chunk_idx}")
    #             reset_chunk_dirty_bits(chunk_dir, self.chunk_size)
    #         # Try loading again with reset bits
    #         return self._safe_load(index)
    #
    #     # Select an item from the clean indices
    ...
    #     except Exception as e:
    #         print(f"Error when modifying the dirty bit: {e}")
    #         traceback.print_exc()
    #
    #     # Load the sample
    #     try:
            ...
            try:
                ...
                if self.state_noise_snr is not None:
                    states += np.random.normal(
                        ...
                        states.shape)
                if data_dict['dataset_name'] in self.dataset_stat:
                    ds_state_mean = np.array(self.dataset_stat[data_dict['dataset_name']]['state_mean'])
                else:
                    # Fallback for new datasets without stats
                    ds_state_mean = np.zeros(states.shape[1], dtype=states.dtype)
                ...
                if self.use_precomp_lang_embed:
                    ...
                    data_dict["lang_embed"] = content["instruction_embedding"]
...
if __name__=="__main__":
    ...
    # text_embedder = T5Embedder(from_pretrained="google/t5-v1_1-xxl",
    #                            model_max_length=config["dataset"]["tokenizer_max_length"], device="cuda:0")
    ...
    print("Single item keys:", test_item.keys())
    print("Sample sizes:")
    ...
    for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                # text_embeds_pre_compute = batch["lang_embeds"].to(dtype=weight_dtype)
                # text_embeds_compute = text_encoder(input_ids=batch["input_ids"], attention_mask=lang_attn_mask)["last_hidden_state"].detach()
